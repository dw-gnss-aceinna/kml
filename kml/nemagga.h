...
#pragma warning (disable:4996)
	...
	void gga_gap_analysis(const char *fname1, const char *fname2)
	{
		TNEMAGGAReader gga;
		gga.ReadGGA(fname1);
		std::vector<TGPGGA>::iterator pGGA;
		double lastFixTime = 0.0;
		double lastFltTime = 0.0;
		int numofrefix = 0;
		int numoflostfix = 0;
		FILE *fout = fopen(fname2, "w");
		for (pGGA = gga.vGGA_.begin(); pGGA != gga.vGGA_.end(); ++pGGA)
		{
			if (pGGA->solType == 4) {
				if (lastFixTime == 0.0)
				{
					lastFixTime = pGGA->time;
				}
				if (lastFltTime == 0.0)
				{
				...
				else
				{
					++numofrefix;
					if (fout) {
						fprintf(fout, "%3i,%6.0f,%5.0f, flt duration\n", numofrefix, pGGA->time, pGGA->time - lastFltTime);
					}
					lastFltTime = 0.0;
				}
			}
			else
			{
				...
				else
				{
					numoflostfix++;
					if (fout) {
						fprintf(fout, "%3i,%6.0f,%5.0f, fix duration\n", numoflostfix, pGGA->time, pGGA->time - lastFixTime);
					}
					lastFixTime = 0.0;
				}
			}
